...
les.[Run Date],
les.[Service Center]
FROM GMHR_LES_FLAT_FILE les
JOIN (SELECT DISTINCT * FROM BOC_LOOKUP_TABLE) boc 
ON CONVERT(VARCHAR(MAX),boc.BOC)  = les.[Object]
JOIN (SELECT DISTINCT * FROM LES_COUNTRY_NAME_LOOKUP) leslookup
ON leslookup.[Country Name 1]=les.[Country Name] 
JOIN (SELECT DISTINCT * FROM  GM_HR_PAYPLAN_LOOKUP) payplan
ON payplan.[Pay Plan]=les.[Salary Plan]
GO